...
const Tbl=({cols,rows}:{cols:string[];rows:CellValue[][]})=><div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontFamily:FONT.mono,fontSize:13}}><thead><tr>{cols.map((c,i)=><th key={i} style={{padding:"2px 1px",textAlign:i===0?"left":"right",color:C.inkLight,fontSize:12,borderBottom:`1px solid ${C.border}`}}>{c}</th>)}</tr></thead><tbody>{rows.map((r,i)=><tr key={i} style={{borderBottom:i<rows.length-1?`1px solid ${C.border}`:"none"}}>{r.map((c,j)=><td key={j} style={{padding:"6px 4px",textAlign:j===0?"left":"right",...(typeof c==="object"&&c!==null?c.s:{})}}>{typeof c==="object"&&c!==null?c.v:c}</td>)}</tr>)}</tbody></table></div>;
...
export default function AheadCalculator(){
  ...
  useEffect(()=>{try{const hash=window.location.hash;const m=hash.match(/[?&]s=([^&]+)/);if(m){const sc=JSON.parse(atob(m[1])) as Scenario;loadScenario(sc);window.location.hash="/ahead";}}catch{}},[]);
