    ...
    try:
        with get_cursor() as cur:
            ...
            rows = cur.execute(f"""
                ...
                revenue AS (
                    SELECT state_code, year, total_tax_collections
                    FROM fact_state_tax_collections
                )
                SELECT s.state_code, s.fiscal_year,
                       ...
                       COALESCE(r.total_tax_collections, 0) AS total_tax_collections,
                       CASE WHEN COALESCE(r.total_tax_collections, 0) > 0
                            THEN ROUND(s.state_share * 100.0 / r.total_tax_collections, 1)
                            ELSE NULL END AS medicaid_pct_of_revenue
                FROM spending s
                LEFT JOIN revenue r ON s.state_code = r.state_code AND s.fiscal_year = r.year
        ...
        with get_cursor() as cur:
            ...
            rows = cur.execute(f"""
                SELECT state_code, fiscal_year,
                       ...
                       ROUND(COALESCE(enhanced_fmap_rate, fmap_rate), 4) AS enhanced_fmap
        ...
        with get_cursor() as cur:
            ...
            rows = cur.execute(f"""
                ...
                revenue AS (
                    ...
                    FROM fact_state_tax_collections
                    WHERE year = (SELECT MAX(year) FROM fact_state_tax_collections)
                ),
                gdp AS (
                ...
                SELECT d.state_code,
                       ...
                       COALESCE(r.total_tax_collections, 0) AS tax_revenue,
                       CASE WHEN COALESCE(r.total_tax_collections, 0) > 0
                            THEN ROUND(s.state_share * 100.0 / r.total_tax_collections, 1)
                            ELSE NULL END AS medicaid_pct_of_revenue,
                       COALESCE(g.gdp_millions, 0) AS state_gdp_millions,
            ...
            rows = cur.execute(f"""
                ...
                budget AS (
                    SELECT s.state_code,
                           ...
                           r.total_tax_collections
                    FROM fact_cms64_multiyear s
                    LEFT JOIN fact_state_tax_collections r ON s.state_code = r.state_code
                        AND r.year = (SELECT MAX(year) FROM fact_state_tax_collections)
                    WHERE s.fiscal_year = (SELECT MAX(fiscal_year) FROM fact_cms64_multiyear)
                      AND s.state_code != 'US'
                    GROUP BY s.state_code, r.total_tax_collections
                ),
                fmap AS (
                    SELECT state_code, fmap_rate
                    FROM fact_fmap_historical
                ...
                SELECT d.state_code,
                       CASE WHEN COALESCE(b.total_tax_collections, 0) > 0
                            THEN ROUND(b.state_share * 100.0 / b.total_tax_collections, 1)
