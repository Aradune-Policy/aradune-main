...
- fact_macpac_enrollment: state_name, fiscal_year, total_enrollment, child, new_adult_group, other_adult, disabled, aged, dual_total, full_benefit (62 rows, FY2023). IMPORTANT: Contains footnote rows where state_name is long text starting with digits or symbols. Always filter: WHERE LENGTH(state_name) < 40 AND state_name NOT LIKE '%Includes%' AND state_name NOT LIKE '%State reported%'
- fact_macpac_spending_per_enrollee: state_name, fiscal_year, total_all, total_full_benefit, child_all, new_adult_all, disabled_all, aged_all (63 rows, FY2023, dollars per FYE enrollee). IMPORTANT: Contains footnote rows. Always filter: WHERE LENGTH(state_name) < 40 AND state_name NOT LIKE '%Includes%' AND state_name NOT LIKE '%State reported%'
...
- fact_macpac_spending_by_state: state_name, fiscal_year, benefits_total, benefits_federal, benefits_state, admin_total, total_medicaid_total (66 rows, FY2024. Amounts in millions.) IMPORTANT: Contains footnote and subtotal rows (e.g. 'Subtotal (states and territories)', 'State Medicaid Fraud Control Units'). Always filter: WHERE LENGTH(state_name) < 40 AND state_name NOT LIKE '%Subtotal%' AND state_name NOT LIKE '%–%'
- fact_macpac_benefit_spending: state_name, fiscal_year, total_benefits, ffs_hospital, ffs_physician, ffs_dental, ffs_nursing_facility, ffs_prescribed_drugs, managed_care, dsh_adjustments (72 rows, FY2024. Amounts in millions.) IMPORTANT: Contains footnote rows. Always filter: WHERE LENGTH(state_name) < 40 AND state_name NOT LIKE '%Includes%' AND state_name NOT LIKE '%Dash%'
