...
export default function TmsisExplorer() {
  ...
  const [bSort, setBS] = useState("gap");
  ...
  const getFsRate = useCallback((state, code) => feeScheds?.states?.[state]?.rates?.[code] || 0, [feeScheds]);
  ...
  const bench = useMemo(() => {
    ...
    let list = base.map(h => {
      ...
      return { ...h, r1, r2, r3, mc, fs1, g1: pD(r1, h.na), g2: has2 && r2>0 ? pD(r2, h.na) : null, g3: has3 && r3>0 ? pD(r3, h.na) : null, fi: (safe(h.na) - r1) * estStateClaims };
    });
    const sf = { gap:(a,b)=>Math.abs(b.g1)-Math.abs(a.g1), high:(a,b)=>b.r1-a.r1, fiscal:(a,b)=>Math.abs(b.fi)-Math.abs(a.fi), code:(a,b)=>a.c.localeCompare(b.c) };
    list.sort(sf[bSort] || sf.gap);
    return list.slice(0, 80);
  }, [codes, s1, s2, s3, bCat, bSort, q, states, getMcRate, getFsRate]);
  ...
  return (
    <div style={{ maxWidth:960,margin:"0 auto",padding:"10px 16px 40px",fontFamily:"Helvetica Neue,Arial,sans-serif",color:A }}>
          ...
          <div style={{ display:"grid",gap:8 }}>
            <Card accent={d1Dev>0?NEG:POS}>
              ...
              <div style={{ display:"grid",gridTemplateColumns:"repeat(3,1fr)",padding:"0 6px 8px" }}>
                <Met l="Spend" v={f$(d1.spend)}/><Met l="Enroll" v={fN(d1.enroll)}/><Met l="Per Cap" v={`$${safe(d1.pe).toLocaleString()}`}/>
                <Met l="FMAP" v={`${d1.fmap}%`}/><Met l="MC%" v={`${d1.mc}%`}/><Met l="Codes" v={codes.filter(h=>h.r&&h.r[s1]!==undefined).length.toLocaleString()}/>
              </div>
            </Card>
            {s2 && s2!==s1 && (() => { const d2=g(s2); const d2Dev=pD(d2.pe,natlPE); const d2vs1=d1.pe>0?((d2.pe-d1.pe)/d1.pe*100):0; return <Card accent={d2Dev>0?NEG:POS}>
              ...
              <div style={{ display:"grid",gridTemplateColumns:"repeat(3,1fr)",padding:"0 6px 8px" }}>
                <Met l="Spend" v={f$(d2.spend)}/><Met l="Enroll" v={fN(d2.enroll)}/><Met l="Per Cap" v={`$${safe(d2.pe).toLocaleString()}`}/>
      ...
      {tab==="rate" && (() => {
        const has2 = s2 && s2 !== s1;
        const has3 = s3 && s3 !== s1 && s3 !== s2;
        const chartData = bench.slice(0, 25);
        ...
        <div style={{ display:"flex",gap:3,alignItems:"center" }}>
          <span style={{ fontSize:8,color:AL,fontFamily:FM,textTransform:"uppercase",letterSpacing:0.5,marginRight:2 }}>Sort by</span>
          {[["gap","Gap vs Natl"],["high","Rate (High→Low)"],["fiscal","Fiscal Impact"],["code","Code"]].map(([k,l])=><Pill key={k} on={bSort===k} onClick={()=>setBS(k)}>{l}</Pill>)}
        </div>
        {chartData.length>0 && <Card x><CH t="Payment Gap" b={compLabel} r={`top ${chartData.length} of ${bench.length}`}/><div style={{ padding:"0 14px 8px" }}>
          <ResponsiveContainer width="100%" height={Math.max(200,chartData.length*(has2?28:22))}>
            <BarChart data={chartData} layout="vertical" margin={{left:130,right:30}}>
              <CartesianGrid strokeDasharray="3 3" stroke={B} horizontal={false}/>
              <XAxis type="number" tick={{fill:AL,fontSize:9,fontFamily:FM}} axisLine={false} tickLine={false} tickFormatter={v=>`${v>0?"+":""}${v.toFixed(0)}%`} domain={([a,b])=>[Math.min(a,-5),Math.max(b,5)]}/>
              <YAxis type="category" dataKey="c" tick={{fill:A,fontSize:8,fontFamily:FM}} axisLine={false} tickLine={false} width={128} tickFormatter={v=>{const x=codes.find(h=>h.c===v);return x?`${v} ${x.d.substring(0,16)}`:v;}}/>
              <ReferenceLine x={0} stroke={AL} strokeDasharray="3 3"/>
              <Tooltip content={<SafeTip render={d=>(<div><div style={{fontWeight:600}}>{d.c} — {d.d}</div><div>{states[s1]?.name||s1}: <b>{f$(d.r1)}</b> · Gap: <span style={{color:d.g1<0?NEG:POS}}>{d.g1>0?"+":""}{d.g1.toFixed(1)}%</span></div>{has2&&d.g2!=null&&<div>{states[s2]?.name||s2}: <b>{f$(d.r2)}</b> · Gap: <span style={{color:d.g2<0?NEG:POS}}>{d.g2>0?"+":""}{d.g2.toFixed(1)}%</span></div>}{has3&&d.g3!=null&&<div>{states[s3]?.name||s3}: <b>{f$(d.r3)}</b> · Gap: <span style={{color:d.g3<0?NEG:POS}}>{d.g3>0?"+":""}{d.g3.toFixed(1)}%</span></div>}<div style={{color:AL,fontSize:9}}>Natl avg: {f$(d.na)}</div></div>)}/>}/>
              <Bar dataKey="g1" barSize={has2?7:10} radius={[0,3,3,0]} name={states[s1]?.name||s1}>{chartData.map((d,i)=><Cell key={i} fill={d.g1<0?NEG:POS}/>)}</Bar>
              {has2&&<Bar dataKey="g2" barSize={7} radius={[0,3,3,0]} name={states[s2]?.name||s2}>{chartData.map((d,i)=><Cell key={i} fill={d.g2==null?"transparent":d.g2<0?"rgba(164,38,44,0.4)":"rgba(46,107,74,0.35)"}/>)}</Bar>}
            </BarChart>
          </ResponsiveContainer>
          <div style={{ display:"flex",gap:10,padding:"4px 0",fontSize:9 }}>
            <span><span style={{ display:"inline-block",width:10,height:10,borderRadius:2,background:POS,verticalAlign:"middle",marginRight:3 }}/>{states[s1]?.name||s1}</span>
            {has2&&<span><span style={{ display:"inline-block",width:10,height:10,borderRadius:2,background:"rgba(46,107,74,0.35)",verticalAlign:"middle",marginRight:3 }}/>{states[s2]?.name||s2}</span>}
            <span style={{ color:AL }}>Gap = % above/below national avg rate per claim</span>
          </div>
        </div></Card>}
        <Card x><CH t="Rate Table" b={compLabel}/><div style={{ padding:"0 14px 8px",overflowX:"auto" }}>
          <table style={{ width:"100%",borderCollapse:"collapse",fontSize:10 }}><thead><tr style={{ borderBottom:`2px solid ${B}` }}>
            {[
              ...
              {h:"Natl",t:"National avg $/claim (T-MSIS)",show:true},
              ...
              {h:"Gap",t:`${states[s1]?.name||s1} % above/below natl avg`,show:true},
            ...
            {bench.map(d=>(
              <tr key={d.c} style={{ borderBottom:`1px solid ${B}`,cursor:"pointer" }} onClick={()=>{setDC(d.c);setTab("code");}}>
                ...
                <td style={{ fontFamily:FM,color:AL }}>{f$(d.na)}</td>
          ...
          <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:4 }}>
            <div style={{ fontSize:9,color:AL }}>Click row → Code Profile · T-MSIS rates = <b>avg paid per claim</b>{mcRates?" · Medicare = PFS non-fac":""}{feeScheds?.states?.[s1]?" · Fee Sched = state Medicaid fee schedule":""} · {bench.length} codes</div>
            <button onClick={()=>{
              const hdr = ["Code","Description","Category",`${states[s1]?.name||s1} (T-MSIS)`];
              if(has2) hdr.push(states[s2]?.name||s2);
              if(has3) hdr.push(states[s3]?.name||s3);
              hdr.push("National Avg");
              if(mcRates) hdr.push("Medicare PFS");
              if(feeScheds?.states?.[s1]) hdr.push(`${states[s1]?.name||s1} Fee Sched`);
              hdr.push("Gap %","Est Impact");
              const rows = bench.map(d=>{
                const r = [d.c,`"${d.d}"`,d.cat,d.r1.toFixed(2)];
                if(has2) r.push(d.r2>0?d.r2.toFixed(2):"");
                if(has3) r.push(d.r3>0?d.r3.toFixed(2):"");
                r.push(d.na.toFixed(2));
                if(mcRates) r.push(d.mc>0?d.mc.toFixed(2):"");
                if(feeScheds?.states?.[s1]) r.push(d.fs1>0?d.fs1.toFixed(2):"");
                r.push(d.g1.toFixed(1),d.fi.toFixed(0));
                return r;
              });
              const csv = [hdr.join(","),...rows.map(r=>r.join(","))].join("\n");
              const blob = new Blob([csv],{type:"text/csv"});
              const a = document.createElement("a"); a.href=URL.createObjectURL(blob); a.download=`medicaid_rates_${s1}.csv`; a.click();
            }} style={{ fontSize:9,color:cB,background:"none",border:`1px solid ${B}`,borderRadius:4,padding:"2px 8px",cursor:"pointer",whiteSpace:"nowrap" }}>Export CSV</button>
          </div>
        </div></Card>
      </div>; })()}
      ...
      {tab==="about" && <div style={{ maxWidth:640,display:"grid",gap:10 }}>
        ...
        <div style={{ fontSize:10,color:AL }}>Aradune T-MSIS Explorer v0.7.0 · Built by <a href="https://aradune.co" style={{ color:cB,textDecoration:"none",fontWeight:600 }}>Aradune</a></div>
      </div>}
